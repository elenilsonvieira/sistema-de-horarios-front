...
export const BoardContainer: React.FC<IntfcBoard> = ({
  label,
  listLesson,
  intervalList,
  change,
  ...
  function getUuidInterval(week: string, gap: string, shift: string) {
    return intervalList.filter(
      (i) =>
        i.gapDTO.gap === gap &&
        i.shiftDTO.shift === shift &&
        i.weekDayDTO.dayOfWeek === week,
    )[0].uuid;
  }
