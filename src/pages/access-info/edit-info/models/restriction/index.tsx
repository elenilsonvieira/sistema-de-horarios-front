        ...
        <Main>
            {restrictionList != null ? (
                ...
                restrictionList.map((restriction, index) => {
                    const idx = index + 1;
                    return (
                        <Row key={restriction.uuid} propertyName={restriction.professor.name}>
                                    ...
                                    {editMode ?
                                        <SelectArea id={'c'+index} change={(event)=>{
                                            ...
                                            if (professorModelList) {
                                                const professorSelect = professorModelList[select.selectedIndex];
                                ...
                                <div className={editMode? 'edit-mode' : ''}>
                                    ...
                                    {editMode ?
                                        <SelectArea id={'c'+index} change={(event)=>{
                                            const select  = event.target;
                                            if (weekDayModelList) {
                                                const weekDaySelect = weekDayModelList[select.selectedIndex];
                                        ...
                                        :
                                        <span className='info'>{restriction.weekDayDTO.displayName}</span>
                                    }
                                </div>
                                <div className={editMode? 'edit-mode' : ''}>
                                    ...
                                    {editMode ?
                                        <SelectArea id={'c'+index} change={(event)=>{
                                            const select  = event.target;
                                            if (shiftModelList) {
                                                const shiftSelect = shiftModelList[select.selectedIndex];
                                        ...
                                        :
                                        <span className='info'>{restriction.shiftDTO.displayName}</span>
                                    }
                                </div>
                                <ActionContainer>
                                    {editMode &&
                                        <EditButtons>
                                            <ButtonDelete  onClickFunction={ async () => {
                                                ...
                                                if(response){
                                                    await restrictionDeleteControllerView(restriction.uuid ? restriction.uuid : "");
                                                    await load();
                                                }
                                            }}/>
